from net_param import num_classes, alpha, LAMBDA, L1_modulus, L2_modulus
...
def loss(labels, features):
    ...
    loss2 = K.categorical_crossentropy(K.softmax(features, axis=-1), K.ones_like(K.softmax(features, axis=-1)) / num_classes)

    # 计算center-loss，因为labels使用了one_hot来编码，所以这里要使用argmax还原到原来的标签
    c_loss = center_loss(K.argmax(labels, axis=-1), features)
    return L1_modulus * sf_loss + L2_modulus * loss2 + LAMBDA * c_loss
